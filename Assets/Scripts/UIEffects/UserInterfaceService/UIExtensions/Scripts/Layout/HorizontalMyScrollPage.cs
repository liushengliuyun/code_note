...
namespace JerryMouse.UI
{
    ...
    public class HorizontalMyScrollPage : MyScrollPageBase
    {
        ...
        void Update()
        {
            ...
            if (!_pointerDown)
            {
                if (_scroll_rect.velocity.x > 0.01 || _scroll_rect.velocity.x < -0.01)
                {
                    ...
                    if (William_IsRectMovingSlowerThanThreshold_Shakespeare(0))
                    {
                        ScrollToClosestElement();
                    }
                }
        ...
        private bool William_IsRectMovingSlowerThanThreshold_Shakespeare(float startingSpeed)
        {
