...
public class ComponentAutoBindToolInspector : Editor
{
    ...
    private SerializedProperty bindComs;
    ...
    private void OnEnable()
    {
        ...
        bindComs = serializedObject.FindProperty("bindComs");
    ...
    private void CheckBindData(Transform child)
    {
        ComponentAutoBindTool autoBindTool = child.GetComponent<ComponentAutoBindTool>();
        ...
        {
            CheckBindData(child.GetChild(i));
        }
    }
    ...
    private void SyncBindComs()
    {
        ...
        for (int i = 0; i < m_BindDatas.arraySize; i++)
        {
            ...
            bindComs.GetArrayElementAtIndex(i).objectReferenceValue = property.objectReferenceValue;
        }
    }
    ...
    private void GenAutoBindCode()
    {
        ...
        using (StreamWriter sw = new StreamWriter($"{codePath}/{className}.Bind.cs"))
        {
            ...
            for (int i = 0; i < m_Target.BindDatas.Count; i++)
            {
                ...
                sw.WriteLine($"\t\t\t{filedName} = autoBindTool.FindComponent<{data.BindCom.GetType().Name}>({i});");
            }
