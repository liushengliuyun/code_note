...
namespace KEngine.UI
{
    ...
    public class UIWindowAsset : MonoBehaviour
    {
        ...
        [HideInInspector] public PanelType PanelType = PanelType.NormalUI;
        ...
        [HideInInspector] public int TabBarId = 0;
        [Range(1, 5)] public int MAX_Atlas = 3;

        /// <summary>
        /// 当前界面包含的图集，在导出UI时会自动赋值，不需要处理
        /// </summary>
        [HideInInspector] public string Atals_arr = "";
        ...
        {
            var findAssets = AssetDatabase.FindAssets("t:SpriteAtlas",
                new string[] { "Assets\\" + KEngineDef.ResourcesBuildDir });
            if (findAssets != null && findAssets.Length >= 1)
            {
                ...
                {
                    YZLog.LogError($"找不到名字为{tag}的SpriteAtlas");
                }
            }
            else
            {
