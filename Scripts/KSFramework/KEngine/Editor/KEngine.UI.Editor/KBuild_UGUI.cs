...
#else
                var prefab =
 PrefabUtility.CreatePrefab(uiPrefabPath, windowAsset.gameObject, ReplacePrefabOptions.Default);
#endif
        ...
        private static void BeforeExportUIPrefab(UIWindowAsset asset)
        {
            ...
            var src_path = scene.path.Replace(scene.name + ".unity", "").Replace(Application.dataPath, "Assets/") +
                           "atlas";
        ...
        private static void OnAfterBuildPlayerEvent(BuildTarget buildTarget, string pathToBuiltProject)
        {
            ...
            if (buildTarget == BuildTarget.Android)
            {
                if (!pathToBuiltProject.EndsWith(".apk"))
                {
                    ...
                    YZLog.Info($"Android Studio工程导出完成，创建Gradle打包脚本到{pathToBuiltProject}");
        ...
        public static void ExportCurrentUI()
        {
            ...
            {
                YZLog.Error("Cannot export in playing mode! Please stop!");
                return;
            }
#if UNITY_4
            var windowAssets = GetUIWIndoeAssetsFromCurrentScene();
...
#endif
                YZLog.Error("Not found UIWindowAsset in scene `{0}`", currentScene);
            }
        ...
        public static void ExportAllUI()
        {
            ...
            {
                YZLog.Info("begin export {0}", uiScene);
        ...
        public static void CreateNewUI(PanelType panelType)
        {
            ...
            if (string.IsNullOrEmpty(uiName) ||
                GameObject.Find(uiName) != null) // default use scene name, if exist create random name
            {
            ...
            GameUtils.ResetLocalTransform(uiPanel.transform);
        ...
        public static void BuildCommonAtlas()
        {
            ...
            var dirs = Directory.GetDirectories(assetPath + KEngineDef.ResourcesEditDir + "/UI/", "atlas_*",
                SearchOption.TopDirectoryOnly);
            foreach (string dir in dirs)
            {
                var arr = dir.Split('/');
                var name = dir.Replace(assetPath, "");
                CreateSpriteAtlas("Assets/" + name, "Assets/" + KEngineDef.ResourcesBuildDir + "/uiatlas/",
                    arr[arr.Length - 1], false);
            }
        }

        public static void CreateSpriteAtlas(string src_path, string dst_path, string atlas_name,
            bool includeInBuild = true)
        {
            if (string.IsNullOrEmpty(src_path) || string.IsNullOrEmpty(dst_path) || string.IsNullOrEmpty(atlas_name))
            {
                YZLog.LogError($"无法创建图集,路径为空. src_path={src_path} ,dst_path={dst_path} ,atlas_name={atlas_name}");
                return;
            }
            ...
            {
                YZLog.Info($"不创建图集,{src_path}下无atlas文件夹");
                return;
            }
            ...
            {
                YZLog.Info($"{src_path}下无图片，不生成图集");
                return;
            }
            ...
            SpriteAtlasUtility.PackAtlases(new SpriteAtlas[] { spriteAtlas }, EditorUserBuildSettings.activeBuildTarget,
                false);
            AssetDatabase.SaveAssets();
            YZLog.Info($"创建图集{spriteAtlas.name}完成，包含{spriteAtlas.spriteCount}个图片");
        }
