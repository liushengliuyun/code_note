...
namespace KEngine.Editor
{
    ...
    public partial class KResourceBuilder
    {
        ...
        {
            YZLog.Info("Start Auto Build... {0}", export.GetType().Name);
            ...
            try
            {
                ...
                if (ext.StartsWith("dir:")) // 目錄下的所有文件，包括子文件夾
                {
                    ...
                    itemArray = Directory.GetFiles(
                        "Assets/" + export.GetResourceBuildDir() + "/" + export.GetDirectory(), newExt,
                        SearchOption.AllDirectories);
                }
                else if (ext == "dir")
                    itemArray = Directory.GetDirectories("Assets/" + export.GetResourceBuildDir() + "/" +
                                                         export.GetDirectory());
                else if (ext == "")
                    itemArray = new string[0];
                else
                    itemArray = Directory.GetFiles(
                        "Assets/" + export.GetResourceBuildDir() + "/" + export.GetDirectory(),
                ...
                YZLog.Info("Finish Auto Build: {0}, ItemsCount: {1}, Used Time: {2}", export.GetType().Name, okItemCount,
                    DateTime.Now - time);
            }
            catch (Exception e)
            {
                YZLog.Error("[Fail] Auto Build... {0}, Exception: {1}, Used Time: {2}, CurrentScene: {3}, Stack: {4}",
                    export.GetType().Name,
