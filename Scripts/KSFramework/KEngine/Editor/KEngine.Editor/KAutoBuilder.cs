...
namespace KEngine.Editor
{
    public class KAutoBuilder
    {
        ...
        /// <param name="opt"></param>
        /// <returns></returns>
        private static string PerformBuild(string outputpath, BuildTargetGroup buildTargetGroup, BuildTarget tag,
            BuildOptions opt)
        {
            ...
            {
                YZLog.LogError("打包失败，可下载更新的包，需要先生成vresion.txt");
                return null;
            }
            ...
            YZLog.Info("Start Build Client {0} to: {1}", tag, Path.GetFullPath(fullPath));
#if xLua
...
#endif
            YZLog.Info("Build App result:{0} ,errors:{1}", buildResult.summary.result, buildResult.summary.totalErrors);
            return fullPath;
        }
        ...
        {
            PerformBuild("Apps/Windows_Dev/KSFramework_Dev.exe", BuildTargetGroup.Standalone,
        ...
        {
            PerformBuild("Apps/Windows/KSFramework.exe", BuildTargetGroup.Standalone, BuildTarget.StandaloneWindows,
                BuildOptions.None);
        }
        ...
        public static void PerformWinIL2CPPBuild()
        {
            ...
            PerformBuild("Apps/WindowsIL2CPP/KSFramework_IL2CPP.exe", BuildTargetGroup.Standalone,
        ...
        public static void PerformiOSBuild()
        {
            ...
            PlayerSettings.applicationIdentifier =
                "com.github.ksframework"; //todo read from config or param to fill appid
            ...
            PlayerSettings.SetGraphicsAPIs(BuildTarget.iOS,
                new GraphicsDeviceType[] { UnityEngine.Rendering.GraphicsDeviceType.Metal });
            ...
            UnityEditor.Rendering.EditorGraphicsSettings.SetTierSettings(BuildTargetGroup.iOS,
                UnityEngine.Rendering.GraphicsTier.Tier3, ts);
#endif
        ...
        [Obsolete("Obsolete")]
        public static string PerformAndroidBuild(string apkName, bool isDevelopment = true, bool isIL2CPP = false,
            bool exportASProject = false)
        {
            ...
            if (isIL2CPP)
            {
                PlayerSettings.SetIl2CppCompilerConfiguration(BuildTargetGroup.Android,
                    isDevelopment ? Il2CppCompilerConfiguration.Debug : Il2CppCompilerConfiguration.Release);
            ...
            ts.renderingPath = RenderingPath.Forward;
            ts.realtimeGICPUUsage = RealtimeGICPUUsage.Low;
            UnityEditor.Rendering.EditorGraphicsSettings.SetTierSettings(BuildTargetGroup.Android,
                UnityEngine.Rendering.GraphicsTier.Tier3, ts);
#endif
    ...
    public class ResourcesSymbolLinkHelper
    {
        ...
        public static void InitABLinkPath()
        {
            if (!Directory.Exists(AssetBundlesLinkPath))
            {
                ...
                YZLog.Info("Create StreamingAssets Bundles Root Dir {0}", AssetBundlesLinkPath);
            }
            ...
            if (!Directory.Exists(path))
            {
                ...
                YZLog.Info("Create StreamingAssets Bundles Platform Dir {0}", path);
            }
        }
        ...
        public static void SymbolLinkResource()
        {
            ...
            GameUtils.ExecuteFile(linkFile, KResourceModule.GetBuildPlatformName());
            YZLog.Info("Link Bundle/Lua/Settings Resources Path");

            var dstPath = Application.streamingAssetsPath + "/" + AppConfig.VersionTxtName;
            if (File.Exists(dstPath)) File.Delete(dstPath);
            File.Copy(AppConfig.VersionTextPath, dstPath);
            YZLog.Info($"Copy version.txt ,File.Exists:{File.Exists(dstPath)}");
