...
namespace KEngine.Editor
{
    ...
    public class SettingModuleEditor
    {
            ...
            if (Directory.Exists(AppConfig.SettingSourcePath))
            {
                ...
                new KDirectoryWatcher(AppConfig.SettingSourcePath, (o, args) =>
                {
                    ...
                    KEditorUtils.CallMainThread(() =>
                    {
                        ...
                        // TODO : COPY Product\Setting tsv to Assets\BundlesRes\Configs tsv.txt 
        ...
        public static void DoCompileSettings(bool force = true, string forceTemplate = null, bool canCustom = true)
        {
            ...
            {
                YZLog.Info("Start Compile to lua");
                var genParam = new GenParam()
                {
                    settingCodeIgnorePattern = AppConfig.SettingCodeIgnorePattern,
                    genCSharpClass = false, genCodeFilePath = null, forceAll = true,
                    ExportLuaPath = AppConfig.ExportLuaPath
                };
                var compilerParam = new CompilerParam()
                {
                    CanExportTsv = false, ExportTsvPath = AppConfig.ExportTsvPath,
                    ExportLuaPath = AppConfig.ExportLuaPath
                };
                results = new BatchCompiler().CompileAll(AppConfig.SettingSourcePath, AppConfig.ExportLuaPath, genParam,
                    compilerParam);
            }
            else
            {
                ...
                {
                    YZLog.Error("Need to KEngineConfig: ExportTsvPath");
                    return;
                }

                YZLog.Info("Start Compile to c#+tsv");
                ...
                results = new BatchCompiler().CompileAll(AppConfig.SettingSourcePath, AppConfig.ExportTsvPath, genParam,
                    compilerParam);
            }
            ...
            YZLog.Info("TableML all Compile ok!\n{0}", sb.ToString());
            // make unity compile
