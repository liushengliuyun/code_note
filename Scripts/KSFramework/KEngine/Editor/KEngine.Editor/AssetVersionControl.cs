...
namespace KEngine.Editor
{
    ...
    public class AssetVersionControl : IDisposable
    {
        ...
        public AssetVersionControl(bool rebuild = false)
        {
            if (Current != null)
            {
                YZLog.Error("New a KAssetVersionControl, but already has annother instance using! Be careful!");
            }
            ...
            YZLog.Warning("================== KAssetVersionControl Begin ======================");
        ...
        public void Dispose()
        {
            ...
            if (BuildedList.Count > 0)
            {
                ...
                YZLog.Info("一共打包了{0}個資源:\n{1}", BuildedList.Count, string.Join("\n", BuildedList.ToArray()));
            }
            else
                YZLog.Info("没有任何需要打包的资源！");
        ...
        private bool DoCheckBuild(string filePath, bool log = true)
        {
            ...
            {
                if (log)
                    YZLog.Error("[DoCheckBuild]Not Found 无法找到文件 {0}", filePath);
            ...
            if (GameUtils.MD5_File(filePath) != assetMd5.MD5)
        ...
        public void MarkBuildVersion(params string[] sourceFiles)
        {
            ...
            foreach (string file in sourceFiles)
            {
                ...
                var nowMd5 = GameUtils.MD5_File(file);
                ...
                if (File.Exists(metaFile))
                {
                    ...
                    var nowMetaMd5 = GameUtils.MD5_File(metaFile);
