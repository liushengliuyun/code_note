    ...
    public class KBytesLoader : AbstractResourceLoader
    {
        ...
        private IEnumerator CoLoad(string url)
        {
            ...
            else
            {
                string _fullUrl;
                var getResPathType =
                    KResourceModule.GetResourceFullPath(url, _loaderMode == LoaderMode.Async, out _fullUrl);
                if (getResPathType == KResourceModule.GetResourceFullPathType.Invalid)
                {
                    YZLog.Error("[HotBytesLoader]Error Path: {0}", url);
                    OnFinish(null);
                    yield break;
                }
                ...
                {
                    YZLog.Error("[HotBytesLoader]Error Load WWW: {0}", url);
                    OnFinish(null);
                    yield break;
                }
