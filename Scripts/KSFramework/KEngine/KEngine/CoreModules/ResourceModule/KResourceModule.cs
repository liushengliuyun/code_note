...
namespace KEngine
{
    ...
    public class KResourceModule : MonoBehaviour
    {
        ...
        static void InitResourcePath()
        {
            ...
            BundlesPathRelative =
                string.Format("{0}/{1}/", AppConfig.StreamingBundlesFolderName, GetBuildPlatformName());
        ...
        {
#if UNITY_EDITOR
            if (KResourceModule.IsEditorLoadAsset && !url.EndsWith(".lua") && !url.EndsWith(AppConfig.SettingExt) &&
                !url.EndsWith(".txt"))
            {
            ...
            out string fullPath, bool raiseError = true)
        {
            ...
            {
                if (raiseError) YZLog.Error($"[Not Found] StreamingAssetsPath Url Resource: {url} ,fullPath:{inAppUrl}");
                fullPath = null;
                return GetResourceFullPathType.Invalid;
            }
        ...
        public static GetResourceFullPathType GetResFullPathType(string fullPath)
        {
            ...
            {
                YZLog.Error("无法识别一个空的资源路径！");
                return GetResourceFullPathType.Invalid;
            }

            if (Application.platform == RuntimePlatform.Android)
                return fullPath.StartsWith("/data/app")
                    ? GetResourceFullPathType.InApp
                    : GetResourceFullPathType.InDocument;
        ...
        public static bool TryGetInAppStreamingUrl(string url, bool withFileProtocol, out string newUrl)
        {
            ...
            if (Application.isEditor)
            {
                ...
                (Application.platform ==
                 RuntimePlatform.Android) // 注意，StreamingAssetsPath在Android平台時，壓縮在apk里面，不要使用File做文件檢查
            {
            ...
            if (Application.isEditor)
            {
                ...
                if (!result)
                {
                    YZLog.Error("[大小写敏感]发现一个资源 {0}，大小写出现问题，在Windows可以读取，手机不行，请改表修改！", url);
                }
            }
        ...
        public static string UnityEditor_EditorUserBuildSettings_activeBuildTarget
        {
            get
            {
                if (Application.isPlaying &&
                    !string.IsNullOrEmpty(_unityEditorEditorUserBuildSettingsActiveBuildTarget))
                {
        ...
        private void Awake()
        {
            ...
            {
                //真机上输出这几个路径
                YZLog.Info("ResourceManager AppBasePath:{0} ,AppBasePathWithProtocol:{1}", AppBasePath,
                    AppBasePathWithProtocol);
                YZLog.Info("ResourceManager AppDataPath:{0} ,AppDataPathWithProtocol:{1}", AppDataPath,
                    AppDataPathWithProtocol);
            }
        }
