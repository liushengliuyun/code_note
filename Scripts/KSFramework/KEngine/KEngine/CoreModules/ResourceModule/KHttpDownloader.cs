...
namespace KEngine
{
    ...
    public class KHttpDownloader : IDisposable
    {
            ...
            int expireDays = 1, int timeout = 10, bool isLog = false)
        {
            ...
            _isLog = isLog;
            GameEx.Instance.StartCoroutine(StartDownload(fullUrl));
        }
            ...
            Action successCallback)
        {
            ...
            if (_useContinue && System.IO.File.Exists(TmpDownloadPath))
            {
                ...
                if (_isLog) YZLog.LogConsole_MultiThread("Resume.... from {0}", lStartPos);
            }
            else
            {
            ...
            try
            {
                ...
                if (_isLog) YZLog.LogConsole_MultiThread("Getting Response : {0}", url);

                //向服务器请求，获得服务器回应数据流 
                using (var response = request.GetResponse()) // TODO: Async Timeout
                {
                    TotalSize = (int)response.ContentLength;
                    if (_isLog) YZLog.LogConsole_MultiThread("Getted Response : {0}", url);
                    if (IsFinished)
                    {
                        throw new Exception(string.Format("Get Response ok, but is finished , maybe timeout! : {0}",
                            url));
                    }
                    else
                    {
                        ...
                        {
                            if (_isLog) YZLog.LogConsole_MultiThread("Start Stream: {0}", url);
                ...
                if (_isLog) YZLog.LogConsole_MultiThread("下载完成: {0}", url);
            ...
            {
                if (_isLog) YZLog.LogConsole_MultiThread("下载过程中出现错误:" + ex.ToString());
                ...
                {
                    if (_isLog) YZLog.LogConsole_MultiThread("删除临时下载文件出错:" + e.Message);
                }
        ...
        public void Dispose()
        {
            if (!FinishedFlag)
            {
                FinishedFlag = true;
                ErrorFlag = true;
                YZLog.LogError(string.Format("[HttpDownloader]Not finish but Dispose: {0}", Url));
            }
        }
    }
}