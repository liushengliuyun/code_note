...
namespace KEngine
{
    ...
    public abstract class AbstractResourceLoader : IAsyncObject
    {
        ...
        private readonly List<AbstractResourceLoader.LoaderDelgate> _afterFinishedCallbacks =
            new List<AbstractResourceLoader.LoaderDelgate>();
            ...
            params object[] initArgs) where T : AbstractResourceLoader, new()
        {
            ...
            {
                YZLog.Error("[{0}:AutoNew]url为空", typeof(T));
                return null;
            }
        ...
        protected virtual void OnFinish(object resultObj)
        {
            ...
            {
                //Dispose();
                YZLog.Trace("[AbstractResourceLoader:OnFinish]时，准备Disposed {0}", Url);
            }
        }
            ...
            if (callback != null)
            {
                if (IsCompleted)
                {
                    if (ResultObject == null)
                        YZLog.LogError("Null ResultAsset {0}", Url);
...
//                IsBeenReleaseNow = true;
        ...
        public virtual void Release()
        {
            if (IsReadyDisposed)
            {
                YZLog.Warning("[{0}]repeat  dispose! {1}, Count: {2}", GetType().Name, this.Url, RefCount);
            }
            ...
            if (RefCount <= 0)
            {
                if (AppConfig.IsLogAbInfo)
                {
                    if (RefCount < 0)
                    {
                        YZLog.Error("[{3}]RefCount< 0, {0} : {1}, NowRefCount: {2}, Will be fix to 0", GetType().Name,
        ...
        public void Dispose()
        {
            ...
            if (!IsForceNew)
            {
                ...
                if (Url != null)
                {
                    ...
                    if (!bRemove)
                    {
                        YZLog.Warning("[{0}:Dispose]No Url: {1}, Cur RefCount: {2}", type.Name, Url, RefCount);
                    }
                }
            }
        ...
        public virtual void ForceDispose()
        {
            ...
            if (RefCount != 1)
            {
                YZLog.Warning("[ForceDisose]Use force dispose to dispose loader, recommend this loader RefCount == 1");
            }
