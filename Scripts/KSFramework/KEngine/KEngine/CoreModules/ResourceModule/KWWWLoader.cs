...
#pragma warning restore 0618
        ...
        {
            if (AppConfig.IsLogAbInfo) YZLog.Info("[Request] WWW, {1}", url);
...
#pragma warning restore 0618
            ...
            if (IsReadyDisposed)
            {
                YZLog.Error("[KWWWLoader]Too early release: {0}", url);
                OnFinish(null);
                yield break;
            }
            ...
            if (!string.IsNullOrEmpty(Www.error))
            {
                ...
                if (url.StartsWith(KResourceModule.GetFileProtocol))
                {
                    ...
                    YZLog.Error("File {0} Exist State: {1}", fileRealPath, System.IO.File.Exists(fileRealPath));
                }

                YZLog.Error("[KWWWLoader:Error]{0} {1}", Www.error, url);
            ...
            if (Application.isEditor)
            {
                ...
                {
                    YZLog.Error("[KWWWLoader]Not Disposed Yet! : {0}", this.Url);
                    yield return null;
                }
            }
        }
