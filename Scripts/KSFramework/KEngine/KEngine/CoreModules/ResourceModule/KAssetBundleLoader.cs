    ...
    public class AssetBundleLoader : AbstractResourceLoader
    {
        ...
        public override void Release()
        {
            if (Application.isEditor)
            {
                ...
                {
                    YZLog.Error("要释放Arial字体！！错啦！！builtinextra:{0}", Url);
                }
            }
        ...
        private void LoadInEditor(string path)
        {
            ...
            Object getAsset = UnityEditor.AssetDatabase.LoadAssetAtPath(
                "Assets/" + KEngineDef.ResourcesBuildDir + "/" + path + ".prefab", typeof(UnityEngine.Object));
            if (getAsset == null)
            {
                YZLog.Error("Asset is NULL(from {0} Folder): {1}", KEngineDef.ResourcesBuildDir, path);
            }
