...
namespace KEngine
{
    ...
    public class AppEngine : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                YZLog.Error("Duplicated Instance Engine!!!");
            }
        ...
        private void Init()
        {
            ...
            {
                IsRootUser =
                    GameUtils.HasWriteAccessToFolder(Application.dataPath); // Root User运行时，能穿越沙盒写DataPath, 以此为依据
                YZLog.Info("====================================================================================");
                YZLog.Info("Application.platform = {0}", Application.platform);
                YZLog.Info("Application.dataPath = {0} , WritePermission: {1}", Application.dataPath, IsRootUser);
                YZLog.Info("Application.streamingAssetsPath = {0} , WritePermission: {1}",
                    Application.streamingAssetsPath, GameUtils.HasWriteAccessToFolder(Application.streamingAssetsPath));
                YZLog.Info("Application.persistentDataPath = {0} , WritePermission: {1}", Application.persistentDataPath,
                    GameUtils.HasWriteAccessToFolder(Application.persistentDataPath));
                YZLog.Info("Application.temporaryCachePath = {0} , WritePermission: {1}", Application.temporaryCachePath,
                    GameUtils.HasWriteAccessToFolder(Application.temporaryCachePath));
                YZLog.Info("Application.unityVersion = {0}", Application.unityVersion);
                YZLog.Info("SystemInfo.deviceModel = {0}", SystemInfo.deviceModel);
                YZLog.Info("SystemInfo.deviceUniqueIdentifier = {0}", SystemInfo.deviceUniqueIdentifier);
                YZLog.Info("SystemInfo.graphicsDeviceVersion = {0}", SystemInfo.graphicsDeviceVersion);
                YZLog.Info("====================================================================================");
            }
        ...
        private IEnumerator DoInitModules(IList<IModuleInitable> modules)
        {
            ...
            foreach (IModuleInitable initModule in modules)
            {
                ...
                if (AppConfig.IsDebugBuild)
                {
                    ...
                    YZLog.Info("Init Module: #{0}# Time:{1}, DiffMem:{2}, NowMem:{3}", initModule.GetType().FullName,
    ...
    public class FpsWatcher
    {
        ...
        public void OnUIUpdate()
        {
            ...
            {
                _cacheMemoryStr = string.Format("(mem:{0:F1}MB)", YZLog.GetMonoUseMemory());
#if USE_UGUI_FPS
