...
namespace Core.Services.ResourceService.Internal
{
    ...
    public class SettingModule : SettingModuleBase
    {
        ...
        {
            byte[] fileContent = API.Facade.ResourceSystem.That.LoadAssetSync<TextAsset>(path.ToLower()).bytes;
        ...
        public static void WatchSetting(string path, System.Action<string> action)
        {
            if (!IsFileSystemMode)
            {
                YZLog.Error("[WatchSetting] Available in Unity Editor mode only!");
                return;
            }
            ...
            var dirPath = Path.GetDirectoryName(KResourceModule.EditorProductFullPath + "/" +
                                                AppConfig.SettingResourcesPath + "/" + path);
            dirPath = dirPath.Replace("\\", "/");
            //if(Application.isEditor) Log.Info($"watch:{path}\n{dirPath}");
            if (!Directory.Exists(dirPath))
            {
                YZLog.Error("[WatchSetting] Not found Dir: {0}", dirPath);
                return;
            }
            ...
            if (!_cacheWatchers.TryGetValue(dirPath, out watcher))
            {
                ...
                YZLog.Info("Watching Setting Dir: {0}", dirPath);
            }
            ...
            {
                YZLog.LogConsole_MultiThread("Setting changed: {0}", e.FullPath);
                action.Invoke(path);
            };
        }
#endif
