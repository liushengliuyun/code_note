...
namespace Core.Services.ResourceService.Internal.UniPooling
{
    internal class GameObjectPool
    {
        ...
        public GameObjectPool(GameObject poolRoot, string location, bool dontDestroy, int initCapacity, int maxCapacity,
            float destroyTime)
        {
        ...
        /// 创建对象池
        /// </summary>
        public void CreatePool(ResourcePackage assetPackage)
        {
            // 加载游戏对象
            AssetHandle = assetPackage.LoadAssetAsync<GameObject>(Location);

            void InitPoolObject(AsyncOperationBase operationBase)
            {
                operationBase.Completed -= InitPoolObject;
                if (operationBase is InstantiateOperation instantiateOperation)
                {
                    instantiateOperation.Result.SetActive(false);
                }
            }
            ...
            for (int i = 0; i < _initCapacity; i++)
            {
                ...
                operation.Completed += InitPoolObject;
        ...
        /// 获取一个游戏对象
        /// </summary>
        public SpawnHandle Spawn(Transform parent, Vector3 position, Quaternion rotation, bool forceClone,
        ...
        private void SetRestoreGameObject(GameObject gameObj)
        {
            if (gameObj != null)
            {
                if (_poolRoot)
                {
                    UniPooling.OnRestore?.Invoke(gameObj);
