...
namespace Jing.TurbochargedScrollList
{
    ...
    public abstract class BaseScrollList : IScrollList
    {
        ...
        protected Vector2 itemDefaultfSize { get; private set; }
        ...
        protected Func<int, GameObject> itemObjProvider;

        //因为需要预先计算content的size
        protected Func<int, Vector2> itemSizeProvider;

        protected ListItemProvider itemDataProvider;

        public int ItemCount
        {
            get { return _itemModels.Count; }
        }
        ...
        /// 当前显示的Item表(key: 数据索引)
        /// </summary>
        protected readonly Dictionary<ScrollListItemModel, ScrollListItem> _showingItems =
            new Dictionary<ScrollListItemModel, ScrollListItem>();
        ...
        protected void InitItem(GameObject itemPrefab = null)
        {
            if (itemPrefab != null)
            {
        ...
        /// <returns></returns>
        protected ScrollListItem CreateItem(ScrollListItemModel model, int dataIdx,
            Dictionary<ScrollListItemModel, ScrollListItem> lastShowingItems)
        {
            ScrollListItem item = null;
            if (lastShowingItems.Count > 0)
            {
                if (lastShowingItems.ContainsKey(model) && lastShowingItems[model].index == dataIdx) //本来就在显示列表中，直接返回
                {
                    ...
                    //因为是同样的物体 ， 所有不需要调用render回调了
                    return item;
                }
                else
                {
                    var keyValuePair = lastShowingItems.KToList().Find(pair =>
                        pair.Key.data.Equals(model.data) && pair.Value.index == dataIdx);

                    if (keyValuePair.Key != null)
                    {
                        item = keyValuePair.Value;
                        lastShowingItems.Remove(keyValuePair.Key);
                    }
                }
            }
            ...
            if (item == null)
            {
                if (_recycledItems.Count > 0)
                {
                    ...
                    while (--idx > -1)
                    {
                        ...
                        if (!tempItem.data.Equals(model.data))
                        {
                            continue;
                        }

                        if (tempItem.index == dataIdx) //以前回收的一个对象，刚好对应的数据一致，直接返回
                        {
                            item = tempItem;
                ...
                if (item == null)
                {
                    var go = itemObjProvider?.Invoke(dataIdx);
                    if (go == null)
                    {
                        go = Object.Instantiate(ItemPrefab, content);
                    }
                    else
                    {
        ...
        {
            var index = _itemModels.Count;
            var model = new ScrollListItemModel(data, itemSizeProvider?.Invoke(index) ?? itemDefaultfSize);
