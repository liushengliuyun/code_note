...
namespace Jing.TurbochargedScrollList
{
    ...
    public class VerticalScrollList : BaseScrollList
    {
        ...
        public VerticalScrollList(ScrollRect scrollRect, Func<int, GameObject> initGameObjProvider,
            Func<int, Vector2> itemSizeProvider, 
            ListItemProvider itemDataProvider, 
            VerticalLayoutSettings layoutSettings = null,
            int topOffset = 0
            )
        {
            layout = layoutSettings ?? new VerticalLayoutSettings();

            InitScrollView(scrollRect);

            itemObjProvider = initGameObjProvider;
            this.itemSizeProvider = itemSizeProvider;
            this.itemDataProvider = itemDataProvider;
            TopOffset = topOffset;
            InitItem();
        }

        public VerticalScrollList(ScrollRect scrollRect, GameObject itemPrefab,
            VerticalLayoutSettings layoutSettings = null)
        {
        ...
        protected override void Refresh(UpdateData updateConfig, out int lastStartIndex)
        {
            ...
            {
                var targetRenderStartPos =
                    (ContentHeight - updateConfig.tempLastContentRect.height) + contentRenderStartPos;
            ...
            var offsetStartY = contentRenderStartPos + TopOffset;
            int dataIdx;
            ...
            Dictionary<ScrollListItemModel, ScrollListItem> lastShowingItems =
                new Dictionary<ScrollListItemModel, ScrollListItem>(_showingItems);

            _showingItems.Clear();

            while (dataIdx < NumItems)
            {
