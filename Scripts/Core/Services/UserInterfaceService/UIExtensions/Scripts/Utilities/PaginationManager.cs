            ...
            if (!isAClick)
            {
                if (ChangeScale)
                {
                    for (int i = 0; i < m_PaginationChildren.Count; i++)
                    {
                        ...
                        m_PaginationChildren[i].transform.localScale = ToggleScale;
                        if (i == pageNo)
                        {
                            m_PaginationChildren[i].transform.localScale *= 1.33f;
                        }
                    }
                }
                
                var mPaginationChild = m_PaginationChildren[pageNo];
                mPaginationChild.isOn = true;
            }
        }
