...
namespace IngameDebugConsole.Scripts
{
	...
	{
		 [SerializeField] private MyButton EnterToolBtn;
...
#pragma warning restore 0649
		...
		private void Awake()
		{
			...
			EnterToolBtn.onClick.AddListener(() =>
			{
				UserInterfaceSystem.That.ShowUI<UITools>();
			} );
		...
		IEnumerator UpdateTime()
		{
			while (true)
			{
				...
				if (TimeUtils.Instance.SetServerTime)
				{
					EnterToolBtn.title = TimeSpan.FromSeconds(TimeUtils.Instance.PassTime).ToString();
				}
			}	
		}
